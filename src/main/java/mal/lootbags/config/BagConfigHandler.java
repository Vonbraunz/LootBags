...
public class BagConfigHandler {
	...
	{
		if (currentBag == null) { LootbagsUtil.LogError(ConfigText.ERROR.getText()+"No active bag at line " + linenum + ".", command); return; }
		if (words.length < 2) { LootbagsUtil.LogError(ConfigText.ERROR.getText()+"$BAGVALUE at line " + linenum + " needs at least one value.", command); return; }
		if (words.length > 3) { LootbagsUtil.LogError(ConfigText.ERROR.getText()+"$BAGVALUE at line " + linenum + " has too many words.", command); return; }
		try {
			...
			LootbagsUtil.LogError(ConfigText.ERROR.getText()+"$BAGVALUE at line " + linenum + ": value is not a number.", command);
		}
	}
