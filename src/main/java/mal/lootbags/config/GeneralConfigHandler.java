...
public class GeneralConfigHandler {
	...
	public static void reloadConfig()
	{
		...
		prop = config.get("Bag Opener", "Opener Cooldown", 100);
		prop.comment = "The cooldown in ticks between bags being opened by the Bag Opener block.  Set to 0 to disable cooldown.";
		LootBags.OPENERMAXCOOLDOWN = prop.getInt();
		if (LootBags.OPENERMAXCOOLDOWN < 0) LootBags.OPENERMAXCOOLDOWN = 0;

		prop = config.get("Bag Storage", "Show Full Bag Count", true);
		prop.comment = "If true, the Bag Storage output slot shows the full count of bags that can be extracted.";
		LootBags.STOREDCOUNT = prop.getBoolean();

		prop = config.get("Bag Storage", "Mekanism Override", false);
		prop.comment = "Experimental fix for Mekanism logistic pipes treating the bag count as individual items to extract.";
